...
CREATE OR REPLACE TRIGGER TRG_RISKINGREDIENT_ID
BEFORE INSERT ON RiskIngredient
FOR EACH ROW
WHEN (NEW.ingr_id IS NULL)
BEGIN
    :NEW.ingr_id := SEQ_RISKINGREDIENT_ID.NEXTVAL;
END;
/

CREATE OR REPLACE TRIGGER TRG_ALLERGYINGREDIENT_ID
BEFORE INSERT ON AllergyIngredient
FOR EACH ROW
WHEN (NEW.ingr_id IS NULL)
BEGIN
    :NEW.ingr_id := SEQ_ALLERGYINGREDIENT_ID.NEXTVAL;
END;
/